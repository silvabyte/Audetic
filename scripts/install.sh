#!/usr/bin/env bash
#
# Audetic user-local installer.
#
#   curl -fsSL https://install.audetic.ai/cli/install.sh | bash
#
...
# waits for it to bind 127.0.0.1:3737, and opens the SPA in your default
# browser. No sudo required — everything lives under your $HOME.
#
# For the legacy system-wide install (binary in /usr/local/bin), see
# `release/cli/latest.sh`.
